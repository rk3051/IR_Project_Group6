...
from pyt_splade.tasks.amp import NullContextManager
from pyt_splade.utils.utils import generate_bow, clean_bow, normalize, pruning
...
class Splade(SiameseBase):
    ...
    def apply_saturation(self, logits, attention_mask):
        ...
        if self.saturation_function == 'log':
            ...
            sigmoid_out = torch.sigmoid(torch.relu(logits))
            adjusted_sigmoid = (sigmoid_out - 0.5) * attention_mask.unsqueeze(-1)
            return adjusted_sigmoid
        elif self.saturation_function == 'squared_inverse':
            return 1 / (torch.square(torch.relu(logits)) + 1) * attention_mask.unsqueeze(-1)
        else:
            raise ValueError("Unsupported saturation function")

    def encode(self, tokens, is_q):
        out = self.encode_(tokens, is_q)["logits"]  # shape (bs, pad_len, voc_size)
        saturated_out = self.apply_saturation(out, tokens["attention_mask"])
        if self.agg == "sum":
            return torch.sum(saturated_out, dim=1)
        elif self.agg == "max":
            values, _ = torch.max(saturated_out, dim=1)
            return values
        else:
